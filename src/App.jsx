...
function MiniCalendar({program, history, todayIdx, startDate}) {
  ...
  // Build 7 days for the displayed week (current cycle week + offset)
  // A "week" here = one full program cycle starting from weekOffset*program.length days away
  const today = new Date(); today.setHours(0,0,0,0);
  const base = new Date(startDate);
  // Find start of current cycle
  const daysSinceStart = Math.floor((today - new Date(base.getFullYear(),base.getMonth(),base.getDate())) / (1000*60*60*24));
  const currentCycleStart = daysSinceStart - todayIdx + weekOffset * program.length;

  const days = program.map((day, i) => {
    const date = new Date(base.getFullYear(), base.getMonth(), base.getDate() + currentCycleStart + i);
    const status = getDayStatus(date, program, history, startDate);
    const isToday = date.toDateString() === today.toDateString();
    return { day, date, status, isToday, i };
  });

  const statusColor = { done:"#4CAF50", partial:"#f59e0b", missed:"#e24b4a", rest:"transparent", future:"transparent" };
  ...
  const onTouchEnd = (e) => {
    ...
    if (Math.abs(dx) > 40) setWeekOffset(o => o + (dx < 0 ? 1 : -1));
    touchStart.current = null;
  };

  const weekLabel = weekOffset === 0 ? "THIS CYCLE" : weekOffset < 0 ? `${Math.abs(weekOffset)} CYCLE${Math.abs(weekOffset)>1?"S":""} AGO` : `IN ${weekOffset} CYCLE${weekOffset>1?"S":""}`;
    ...
    <div className="card" style={{userSelect:"none"}}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:4}}>
          <button onClick={()=>setWeekOffset(o=>o-1)} style={{background:"var(--s2)",border:"1px solid var(--border)",borderRadius:6,width:24,height:24,cursor:"pointer",color:"var(--muted)",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
          {weekOffset!==0&&<button onClick={()=>setWeekOffset(0)} style={{background:"var(--s2)",border:"1px solid var(--border)",borderRadius:6,padding:"0 6px",height:24,cursor:"pointer",color:"var(--muted)",fontSize:10}}>NOW</button>}
          <button onClick={()=>setWeekOffset(o=>o+1)} style={{background:"var(--s2)",border:"1px solid var(--border)",borderRadius:6,width:24,height:24,cursor:"pointer",color:"var(--muted)",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:`repeat(${program.filter(d=>!d.isRest).length},1fr)`,gap:4}}>
        {days.filter(({day})=>!day.isRest).map(({day,date,status,isToday,i})=>{
          const col = statusColor[status] || (day.color+"cc");
          const bg = statusBg[status] || "var(--s2)";
          return(
            <div key={i} style={{
              ...
              <div style={{fontFamily:"var(--df)",fontSize:14,color:status==="future"?day.color+"88":col}}>
                {day.label?.[0]?.toUpperCase()??"?"}
              </div>
              <div style={{fontSize:9,color:"var(--muted)"}}>
                {date.getDate()}/{date.getMonth()+1}
              </div>
              {status==="done"&&<div style={{width:5,height:5,borderRadius:"50%",background:"#4CAF50"}}/>}
              {status==="partial"&&<div style={{width:5,height:5,borderRadius:"50%",background:"#f59e0b"}}/>}
              {status==="missed"&&<div style={{width:5,height:5,borderRadius:"50%",background:"#e24b4a"}}/>}
            </div>
          );
        })}
      </div>
      <div style={{display:"flex",gap:12,marginTop:8,fontSize:10,color:"var(--muted)"}}>
        <span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:6,height:6,borderRadius:"50%",background:"#4CAF50",display:"inline-block"}}/> Done</span>
        <span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:6,height:6,borderRadius:"50%",background:"#f59e0b",display:"inline-block"}}/> Partial</span>
        <span style={{display:"flex",alignItems:"center",gap:3}}><span style={{width:6,height:6,borderRadius:"50%",background:"#e24b4a",display:"inline-block"}}/> Missed</span>
      </div>
    </div>
  );
}
          ...
          return(<div key={day.id} className="prog-day-card" style={{borderColor:isT?day.color:undefined}}
            ...
            <div className="prog-day-header">
              ...
              <div className="prog-day-actions">
                ...
                <button className="btn-icon" style={{color:"#cc6666"}} onClick={()=>{if(window.confirm(`Delete "${day.label}"?`)){setProgram(p=>p.filter(d=>d.id!==day.id));}}}><Ic.Trash/></button>
              </div>
            </div>
            ...
            {day.isRest&&<div style={{padding:"8px 14px 10px",fontSize:13,color:"var(--muted)"}}>Rest day · Edit to add exercises.</div>}
          </div>);
        })}
    ...
    {tab==="history"&&<>
      ...
      <div className="scroll">
        ...
        {[...history].reverse().map((s,i)=>{ const day=program.find(d=>d.id===s.dayKey); return(<div key={i} className="hist-card" onClick={()=>setHistDetail(s)}>
          ...
          {s.exercises?.filter(e=>!e.skipped).slice(0,3).map((ex,j)=>(<div key={j} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"var(--muted2)",padding:"2px 0"}}><span>{ex.name}</span><span>{ex.weight}kg · {ex.sets?.filter(st=>!st.skipped&&st.reps).map(st=>st.reps).join("/")}</span></div>))}
          ...
          {s.note&&<div style={{marginTop:6,fontSize:12,color:"var(--muted2)",fontStyle:"italic"}}>📝 {s.note}</div>}
        </div>); })}
      </div>
    </>}
...
// ── SHARED EXERCISE TABLE (editor + session + history) ────────
        ...
        {sets.map((s, si) => {
          ...
          return (
            <tr key={si} className={`set-row ${s.done?"row-done":""} ${s.skipped?"row-skip":""} ${isW&&!s.done?"row-warmup":""} ${isD&&!s.done?"row-drop":""}`}>
              ...
              {/* Previous */}
              <td><span className="set-prev">{s.prevLabel ?? `${s.targetWeight??ex?.weight??0}kg×${s.targetReps??ex?.reps??0}`}</span></td>
              {/* KG */}
              <td>
                {editable ? (
                  ...
                  {onRemoveSet ? (
                    <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
                      <input type="number" className="set-field"
                        ...
                        title="Rest (s)"
                      />
                      <div className="set-skip-x" onClick={()=>onRemoveSet(si)} title="Remove set">✕</div>
                    </div>
                  ) : (
                    <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
    ...
    <div className="modal-title-row">
      <div>
        ...
        <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>
          {new Date(session.date).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})} · {session.duration}min
        </div>
      </div>
      <button className="btn-ghost" onClick={onClose}>Close</button>
    </div>
    {session.note&&<div style={{background:"var(--s2)",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:13,color:"var(--muted2)",fontStyle:"italic"}}>📝 {session.note}</div>}
...
function WorkoutSession({workout,history,onFinish,onBack,startRest,restEnabled}){
  ...
  const initSets=(ex)=>Array.from({length:ex.sets},(_,si)=>{
    ...
    const prevLabel=lastSet&&!lastSet.skipped&&lastSet.reps?`${lastSet.weight??lastEx.weight}kg×${lastSet.reps}`:null;
    return {
      setType:(ex.setTypePerSet||[])[si]??"normal",
      targetReps:(ex.repsPerSet||[])[si]??ex.reps,
      targetWeight:(ex.weightPerSet||[])[si]??ex.weight,
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
      {exStates.map((exState,ei)=>{
        ...
        return(<div key={ex.id}>
          {pg&&!exState.skipped&&<div className={`ai-banner ${pg.type==="increase"?"good":""}`}>{pg.type==="increase"?`🔥 Hit all reps 2 sessions in a row! Try ${(exState.weight+pg.amount).toFixed(2)}kg (+${pg.amount}kg)`:`💡 You struggled last session — stay at ${exState.weight}kg`}</div>}
  ...
  return(<div style={{display:"flex",flexDirection:"column",minHeight:"100svh"}}>
    <div className="w-header">
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:12,color:"var(--muted)"}}>{new Date(session.date).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})} · {session.duration}min</div>
      </div>
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
      ...
      {session.note&&<div style={{margin:"0 14px 14px",padding:"10px 13px",background:"var(--s2)",borderRadius:10,fontSize:13,color:"var(--muted2)",fontStyle:"italic"}}>📝 {session.note}</div>}
    </div>
  </div>);
}
