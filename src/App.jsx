...
export default function App() {
  ...
  const addDay    = ()=>setProgram(p=>[...p,{id:uid(),label:"New Day",shortLabel:"New",color:"#888",isRest:true,exercises:[]}]);
      ...
      <div className="scroll">
        ...
        <div className="card">
          ...
          <div className="week-grid" style={{gridTemplateColumns:`repeat(${program.length},1fr)`}}>
            {program.map((day,i)=>{ const isT=i===todayIdx; const done=history.some(h=>h.dayKey===day.id&&new Date(h.date)>=weekStart); return(<div key={day.id} className={`wdot ${day.isRest?"rdot":""} ${done?"done":""} ${isT?"today":""}`} title={day.label} style={!day.isRest?{color:done?"#88cc88":day.color+"cc"}:{}}>{day.isRest?"·":day.shortLabel?.[0]??"?"}</div>); })}
          </div>
        </div>
      </div>
    </>}
...
function DayEditor({day,onClose,onSave,onAddEx,onRemoveEx,onUpdateEx}){
  ...
  return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
      <div><div className="form-lbl">Day name</div><input className="form-input" value={label} onChange={e=>setLabel(e.target.value)}/></div>
      <div><div className="form-lbl">Short label</div><input className="form-input" value={short} onChange={e=>setShort(e.target.value)} maxLength={6}/></div>
    </div>
    ...
    {day.exercises.map(ex=>(<div key={ex.id} style={{background:"var(--s2)",borderRadius:10,padding:12,marginBottom:8}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:8}}>
        {[{l:"Sets",f:"sets",v:ex.sets},{l:"Reps",f:"reps",v:ex.reps},{l:"Weight(kg)",f:"weight",v:ex.weight}].map(({l,f,v})=>(<div key={f}><div style={{fontSize:10,color:"var(--muted)",marginBottom:3}}>{l}</div><input type="number" className="mini-input" value={v} onChange={e=>{const val=parseFloat(e.target.value)||0;onUpdateEx(ex.id,{[f]:val,...(f==="sets"?{restPerSet:Array.from({length:val},(_,i)=>(ex.restPerSet||[])[i]??ex.rest??120)}:{})});}}/></div>))}
      </div>
      <div style={{fontSize:10,color:"var(--muted)",marginBottom:4}}>REST PER SET (seconds)</div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {Array.from({length:ex.sets},(_,si)=>(<div key={si} style={{flex:1,minWidth:50}}><div style={{fontSize:10,color:"var(--muted2)",marginBottom:3,textAlign:"center"}}>Set {si+1}</div><input type="number" className="mini-input" value={(ex.restPerSet||[])[si]??ex.rest??120} onChange={e=>{const rps=[...(ex.restPerSet||Array.from({length:ex.sets},()=>ex.rest??120))];rps[si]=parseFloat(e.target.value)||0;onUpdateEx(ex.id,{restPerSet:rps});}}/></div>))}
      </div>
    </div>))}
    ...
    <button className="btn-accent" onClick={()=>onSave({label,shortLabel:short,color,exercises:day.exercises})}>SAVE</button>
...
function WorkoutSession({workout,history,onFinish,onBack,startRest,restEnabled}){
  ...
  const onRepsBlur=(ei,si)=>{
    ...
    if(!isNaN(r)&&r>0&&restEnabled&&!(isLastSet&&isLastEx)){const restSecs=(ex.restPerSet&&ex.restPerSet[si]!=null)?ex.restPerSet[si]:ex.rest;startRest(restSecs);}
  };
        ...
        return(<div key={ex.id}>
          ...
          <div className={`ex-card ${exState.skipped?"ex-skip":allSD?"ex-done":""}`}>
            <div className="ex-chead">
              <div><div className="ex-cname">{ex.name}</div><div className="ex-cmeta">{ex.sets} sets × {ex.reps} reps{restEnabled&&ex.restPerSet?` · ${ex.restPerSet.join("/")}s rest`:restEnabled?` · ${ex.rest}s rest`:""}</div></div>
            ...
            {!exState.skipped&&<>
              <div className="weight-row">
                ...
                <div><div className="w-num">{exState.weight}kg</div><div className="w-lbl">WEIGHT</div></div>
                <div className="w-btn" onClick={()=>changeWeight(ei,1)}>+</div>
              </div>
              <div className="sets-row">
                {exState.sets.map((s,si)=>(<div key={si} className={`set-box ${s.done?"set-done":""} ${s.skipped?"set-skip":""}`}>
                  ...
                  <input className="set-input" type="number" placeholder={String(ex.reps)} value={s.reps} disabled={s.skipped} onChange={e=>onRepsChange(ei,si,e.target.value)} onBlur={()=>onRepsBlur(ei,si)}/>
                </div>))}
              </div>
            </>}
          </div>
