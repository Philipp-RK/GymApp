...
// âââ SET TYPE HELPERS ââââââââââââââââââââââââââââââââââââââââ
...
function getSetLabel(sets, si) {
  ...
  if (t === "drop")   return "D";
  // Count only normal sets up to this index
  let n = 0;
  for (let i = 0; i <= si; i++) {
    const st = sets[i]?.setType ?? "normal";
    if (st === "normal") n++;
  }
  return String(n);
}
...
function useDragList(items, setItems) {
  ...
  const onDragStart = (e, i) => {
    dragIdx.current  = i;
    dragItem.current = items[i];
    e.dataTransfer.effectAllowed = "move";
    // slight delay so the ghost image renders before we modify state
  };
  const onDragEnter = (e, i) => {
    e.preventDefault();
    if (overIdx.current === i) return;
    overIdx.current = i;
  };
...
// âââ SHARED SET ROW COMPONENT ââââââââââââââââââââââââââââââââ
...
function MiniCalendar({program, history, todayIdx, startDate}) {
  ...
  // Always show exactly 7 calendar days (Mon-Sun style, but program-aligned)
  // weekOffset=0 = current week containing today
  const today = new Date(); today.setHours(0,0,0,0);
  const base = new Date(startDate);
  const baseMidnight = new Date(base.getFullYear(), base.getMonth(), base.getDate());
  
  // Start of the displayed week: current week start + offset weeks
  // "Week start" = today - todayIdx (start of current program cycle) + weekOffset*7
  const cycleStart = Math.floor(Math.floor((today - baseMidnight) / (1000*60*60*24)) / program.length) * program.length;
  // Map to calendar weeks: find Monday of current week
  const dayOfWeek = today.getDay() === 0 ? 6 : today.getDay() - 1; // 0=Mon
  const weekMondayOffset = dayOfWeek; // days since Monday

  const days = Array.from({length: 7}, (_, i) => {
    const date = new Date(today);
    date.setDate(today.getDate() - weekMondayOffset + i + weekOffset * 7);
    const status = getDayStatus(date, program, history, startDate);
    ...
    <div className="card" style={{userSelect:"none",touchAction:"pan-y"}}
        ...
        {weekOffset !== 0 && (
          <button onClick={()=>setWeekOffset(0)}
            style={{background:"var(--s2)",border:"1px solid var(--border)",borderRadius:6,
              padding:"2px 8px",cursor:"pointer",color:"var(--muted)",fontSize:11}}>
            NOW
          </button>
        )}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>
        {days.map(({day, date, status, isToday}, i) => {
          ...
          const letterColor = status === "done" ? "#4CAF50"
            : status === "partial" ? "#f59e0b"
            : status === "missed" ? "#e24b4a"
            : (day?.color + "99");
          return (
            <div key={i} style={{
...
function SetTypePicker({sets, si, onSelect}) {
  ...
  const handleOpen = () => {
    ...
    if (!rect) return;
    // Open above if not enough space below, otherwise below
  ...
  return (
    <>
      <div ref={btnRef} onClick={handleOpen}
        style={{width:32,height:32,borderRadius:8,background:typeBg,border:`1.5px solid ${typeColor}44`,
          display:"flex",alignItems:"center",justifyContent:"center",
          fontFamily:"var(--df)",fontSize:16,color:typeColor,
          cursor:"pointer",userSelect:"none",flexShrink:0,transition:"all .15s"}}>
        {label}
      </div>
      {pos && (
        <div className="type-picker-popup" style={{top:pos.top,left:pos.left}} onClick={e=>e.stopPropagation()}>
          {options.map(o => (
            <div key={o.type} className="type-picker-item"
              ...
              {(s.setType??"normal") === o.type && <span style={{marginLeft:"auto",color:o.color}}>â</span>}
            </div>
          ))}
        </div>
      )}
...
function SetRow({ sets, si, editable=false, onTypeSelect, children }) {
  const s = sets[si];
  const isWarmup = s.setType === "warmup";
  const isDrop   = s.setType === "drop";
  const typeColor = isWarmup ? "#f59e0b" : isDrop ? "#a78bfa" : s.done ? "#4CAF50" : "#FF6B35";
  const typeBg    = isWarmup ? "rgba(245,158,11,.15)" : isDrop ? "rgba(167,139,250,.15)" : s.done ? "rgba(76,175,80,.15)" : "var(--s3)";
  return (
    <tr className={`set-row ${s.done?"row-done":""} ${s.skipped?"row-skip":""} ${isWarmup&&!s.done?"row-warmup":""} ${isDrop&&!s.done?"row-drop":""}`}>
      <td style={{paddingLeft:14,paddingRight:6,width:40}}>
        {editable && onTypeSelect
          ? <SetTypePicker sets={sets} si={si} onSelect={t=>onTypeSelect(si,t)}/>
          : <div style={{width:32,height:32,borderRadius:8,background:typeBg,border:`1.5px solid ${typeColor}44`,
              display:"flex",alignItems:"center",justifyContent:"center",
              fontFamily:"var(--df)",fontSize:16,color:typeColor,userSelect:"none",flexShrink:0}}>
              {getSetLabel(sets,si)}
            </div>
        }
      </td>
...
.type-picker-popup{position:absolute;left:0;top:38px;background:var(--s1);border:1px solid var(--border2);border-radius:12px;z-index:999;padding:5px;min-width:130px;box-shadow:0 8px 24px rgba(0,0,0,.7);}
...
/* CHAT */
.chat-wrap{display:flex;flex-direction:column;height:100svh;overflow:hidden;}
.chat-msgs{flex:1;overflow-y:auto;padding:14px 14px 8px;display:flex;flex-direction:column;gap:8px;min-height:0;overflow-x:hidden;}
.msg{max-width:86%;}
...
.chat-bar{flex-shrink:0;display:flex;gap:8px;padding:10px 14px;background:var(--s1);border-top:1px solid var(--border);padding-bottom:calc(10px + env(safe-area-inset-bottom,0px));}
...
export default function App() {
  ...
  const todayIdx=startDate?(()=>{
    ...
    const now=new Date();
    // Normalize both to midnight local time to avoid timezone offset errors
  ...
  return(<><style>{CSS}</style><div className="app">
    ...
    {/* HOME */}
    {tab==="home"&&<>
      <div className="phdr"><div className="hdr-row"><div><h1>GRIND</h1><p>Hey {user.name?.split(" ")[0]} ð</p></div><div className="streak-chip">ð¥ {streak}</div></div></div>
  ...
  // First working set weight (normal or drop, skip warmup)
  const firstWorkingSet = (ex.setTypePerSet||[]).findIndex(t=>t==="normal"||t==="drop");
  const displayWeight = firstWorkingSet >= 0
    ? ((ex.weightPerSet||[])[firstWorkingSet] ?? ex.weight)
    : ex.weight;
  const displayReps = firstWorkingSet >= 0
    ? ((ex.repsPerSet||[])[firstWorkingSet] ?? ex.reps)
    : ex.reps;
  return(
    <div key={ex.id} className="ex-row" style={{alignItems:"center"}}>
      <div style={{fontWeight:500,flex:1}}>{ex.name}</div>
      <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
        {pg?.type==="increase"&&<span className="prog-pill">â +{pg.amount}kg</span>}
        <span style={{fontSize:12,color:"var(--muted2)",whiteSpace:"nowrap"}}>{displayWeight}kg Â· {displayReps} reps</span>
      </div>
    </div>
  );
})}</div>
            <button className="btn-accent" onClick={()=>setActiveWorkout(todayDay)}>START WORKOUT</button>
          </>:<p style={{color:"var(--muted2)",marginTop:8,fontSize:14}}>Recovery is part of the process. Rest up ð</p>}
        </div>
      ...
      <div className="scroll">
        {program.map((day,i)=>{
          ...
          return(<div key={day.id} className="prog-day-card" style={{borderColor:isT?day.color:undefined}}
            ...
            {day.exercises.map(ex=>(<div key={ex.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"5px 14px",borderBottom:"1px solid var(--border)"}}><span>{ex.name}</span><span style={{color:"var(--muted)"}}>{ex.sets}Ã{ex.reps} Â· {ex.weight}kg</span></div>))}
            ...
            {day.isRest&&<div style={{padding:"8px 14px 10px",fontSize:13,color:"var(--muted)"}}>Rest day Â· Edit to add exercises.</div>}
          </div>);
        })}
      ...
      <div className="scroll">
        ...
        {[...history].reverse().map((s,i)=>{ const day=program.find(d=>d.id===s.dayKey); return(<div key={i} className="hist-card" onClick={()=>setHistDetail(s)}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <div><div style={{fontWeight:600,color:day?.color}}>{day?.label??s.dayKey}</div><div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>{new Date(s.date).toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})} Â· {s.duration}min</div></div>
            <div style={{fontSize:11,color:"var(--muted)",alignSelf:"center"}}>View â</div>
          </div>
          {s.exercises?.filter(e=>!e.skipped).slice(0,3).map((ex,j)=>(<div key={j} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"var(--muted2)",padding:"2px 0"}}><span>{ex.name}</span><span>{ex.weight}kg Â· {ex.sets?.filter(st=>!st.skipped&&st.reps).map(st=>st.reps).join("/")}</span></div>))}
          ...
          {s.note&&<div style={{marginTop:6,fontSize:12,color:"var(--muted2)",fontStyle:"italic"}}>ð {s.note}</div>}
        </div>); })}
      </div>
    </>}
    ...
    {tab==="settings"&&<>
      ...
      <div className="scroll">
        ...
        <div className="card"><div className="ctitle" style={{fontSize:14}}>GOOGLE INTEGRATION</div><p style={{fontSize:13,color:"var(--muted2)",marginBottom:12,lineHeight:1.6}}>Sync workouts to Google Sheets and add calendar reminders.</p><button className="btn-accent" style={{fontSize:15,padding:12,marginBottom:8}} onClick={handleConnectSheets}>{sheetId?"â SHEETS CONNECTED":"CONNECT GOOGLE SHEETS"}</button><button className="btn-accent" style={{fontSize:15,padding:12,background:"#1a2a3a",color:"#88aacc"}} onClick={handleCalendar}>SET UP CALENDAR REMINDERS</button></div>
...
// ââ SHARED EXERCISE TABLE (editor + session + history) ââââââââ
  ...
  const thead = (
    <thead className="set-table-head">
      <tr>
        ...
        <th>KG</th>
        <th>REPS</th>
        {editable && onRemoveSet && <th style={{textAlign:"right",paddingRight:10,fontSize:10,letterSpacing:.3}}>REST (s)</th>}
        {editable && !onRemoveSet && <th style={{textAlign:"right",paddingRight:10}}></th>}
      </tr>
    </thead>
  );
          ...
          return (
            <tr key={si} className={`set-row ${s.done?"row-done":""} ${s.skipped?"row-skip":""} ${isW&&!s.done?"row-warmup":""} ${isD&&!s.done?"row-drop":""}`}>
              ...
              <td style={{paddingLeft:14,paddingRight:4,width:44}}>
                {editable && onTypeSelect
                  ? <SetTypePicker sets={sets} si={si} onSelect={t=>onTypeSelect(si,t)}/>
                  : <div style={{width:32,height:32,borderRadius:8,background:typeBg,
                      border:`1.5px solid ${typeColor}55`,display:"flex",alignItems:"center",
                      justifyContent:"center",fontFamily:"var(--df)",fontSize:16,color:typeColor,
              ...
              {/* Previous */}
              <td><span className="set-prev">{s.prevLabel ?? `${s.targetWeight??ex?.weight??0}kgÃ${s.targetReps??ex?.reps??0}`}</span></td>
              {/* KG */}
              <td>
                {editable ? (
                  <input className="set-field" type="number" value={s.weight??""}
                    onChange={e=>onWeightChange&&onWeightChange(si,e.target.value)}
              ...
              <td>
                {editable ? (
                  <input className="set-field" type="number" placeholder={String(s.targetReps??ex?.reps??"")}
                    value={s.reps??""}
                    onChange={e=>onRepsChange&&onRepsChange(si,e.target.value)}
                    onBlur={()=>onFieldBlur&&onFieldBlur(si)}
                    disabled={s.skipped} style={{width:58}}/>
                ) : (
                  <span style={{fontWeight:600,fontSize:15,color:s.done?"#88cc88":"var(--text)"}}>{s.reps||"â"}</span>
                )}
              </td>
                ...
                <td style={{paddingRight:10}}>
                  {onRemoveSet ? (
                    <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
                      <input type="number" className="set-field"
                        value={s.rest??ex?.rest??120}
                        onChange={e=>onRestChange&&onRestChange(si,parseInt(e.target.value)||0)}
                        style={{width:52,fontSize:13}}
                        title="Rest (s)"
                      />
                      <div className="set-skip-x" onClick={()=>onRemoveSet(si)} title="Remove set">â</div>
                    </div>
                  ) : (
                    <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
...
// ââ DAY EDITOR âââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}>
    <div className="modal-title-row">
      <div>
        ...
        <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>
          {new Date(session.date).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})} Â· {session.duration}min
        </div>
      </div>
      <button className="btn-ghost" onClick={onClose}>Close</button>
    </div>
    {session.note&&<div style={{background:"var(--s2)",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:13,color:"var(--muted2)",fontStyle:"italic"}}>ð {session.note}</div>}
    {session.exercises?.filter(e=>!e.skipped).map((ex,ei)=>{
      ...
      return(<div key={ei} style={{background:"var(--s2)",borderRadius:12,marginBottom:10,overflow:"hidden"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"36px 1fr 72px 72px",gap:6,padding:"6px 12px",borderBottom:"1px solid var(--border)"}}>
          {["SET","","KG","REPS"].map((h,i)=>(<div key={i} style={{fontSize:10,color:"var(--muted)",fontWeight:600,letterSpacing:.4,textAlign:i>1?"center":"left"}}>{h}</div>))}
        </div>
        {ex.sets?.map((s,si)=>{
          if(s.skipped) return null;
          const lbl=getSetLabel(types,si);
          const t=types[si]??"normal";
          return(<div key={si} style={{display:"grid",gridTemplateColumns:"36px 1fr 72px 72px",gap:6,padding:"9px 12px",borderBottom:"1px solid var(--border)",alignItems:"center",background:t==="warmup"?"rgba(245,158,11,0.05)":t==="drop"?"rgba(167,139,250,0.05)":"transparent"}}>
            <div className={`set-type-chip ${t}`} style={{pointerEvents:"none",color:t==="warmup"?"#f59e0b":t==="drop"?"#a78bfa":"var(--muted2)"}}>{lbl}</div>
            <div/>
            <div style={{textAlign:"center",fontSize:16,fontWeight:600,color:"#88cc88"}}>{s.weight??ex.weight}</div>
            <div style={{textAlign:"center",fontSize:16,fontWeight:600,color:"#88cc88"}}>{s.reps}</div>
          </div>);
        })}
      </div>);
    })}
...
function DayEditor({day,onClose,onSave,onAddEx,onRemoveEx,onUpdateEx,onDelete}){
  ...
  return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}>
    <div className="modal-title-row">
      ...
      <button onClick={onClose} style={{background:"var(--s2)",border:"1px solid var(--border2)",borderRadius:8,width:32,height:32,cursor:"pointer",color:"var(--text)",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>â</button>
      </div>
    </div>
      ...
      return(
        <div key={ex.id} style={{background:"var(--s2)",borderRadius:12,marginBottom:10,overflow:"hidden"}}>
          ...
          <ExerciseSetTable ex={ex} sets={sets} editable={true}
            onTypeSelect={(si,t)=>{
              const a=[...(ex.setTypePerSet||Array(ex.sets).fill("normal"))];
              a[si]=t;
              onUpdateEx(ex.id,{setTypePerSet:a});
            }}
            ...
            onRemoveSet={(si)=>{
              ...
              onUpdateEx(ex.id,{
                sets:ex.sets-1,
                setTypePerSet:splice(ex.setTypePerSet,"normal"),
                repsPerSet:splice(ex.repsPerSet,ex.reps),
                weightPerSet:splice(ex.weightPerSet,ex.weight),
                restPerSet:splice(ex.restPerSet,ex.rest??120),
              });
            }}
            onCheck={null}
...
// ââ WORKOUT SESSION ââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <div style={{display:"flex",gap:0,background:"var(--s2)",borderBottom:"1px solid var(--border)"}}>
      <div style={{flex:1,padding:"10px 16px",borderRight:"1px solid var(--border)"}}>
        ...
        <div style={{fontFamily:"var(--df)",fontSize:20,letterSpacing:1,color:"var(--text)"}}>{fmtTime(elapsed)}</div>
      </div>
      <div style={{flex:1,padding:"10px 16px"}}>
        <div style={{fontSize:10,color:"var(--muted)",letterSpacing:.4,textTransform:"uppercase",marginBottom:2}}>Volume</div>
        <div style={{fontFamily:"var(--df)",fontSize:20,letterSpacing:1,color:"var(--text)"}}>{totalVol > 0 ? `${totalVol.toLocaleString()}kg` : "â"}</div>
      </div>
    </div>
  );
}
...
function WorkoutSession({workout,history,onFinish,onBack,startRest,restEnabled}){
  ...
  // Build initial set states from program definition
  const initSets=(ex)=>Array.from({length:ex.sets},(_,si)=>{
    ...
    const prevLabel=lastSet&&!lastSet.skipped&&lastSet.reps?`${lastSet.weight??lastEx.weight}kgÃ${lastSet.reps}`:null;
    return {
      setType:(ex.setTypePerSet||[])[si]??"normal",
      targetReps:(ex.repsPerSet||[])[si]??ex.reps,
      targetWeight:(ex.weightPerSet||[])[si]??ex.weight,
      weight:String((ex.weightPerSet||[])[si]??ex.weight),
      reps:"",prevLabel,skipped:false,done:false
    };
  });
  ...
  const handleCheck=(ei,si)=>{
    ...
    // If marking done with no reps entered, fill in the target reps
  ...
  const handleAddSet=(ei)=>{
    const ex=workout.exercises[ei];
    setExStates(s=>s.map((exSt,i)=>{
      if(i!==ei) return exSt;
      const last=exSt.sets[exSt.sets.length-1];
      return {...exSt,sets:[...exSt.sets,{...last,reps:"",done:false,skipped:false}]};
    }));
  };
  ...
  return(<div style={{display:"flex",flexDirection:"column",minHeight:"100svh"}}>
    <div className="w-header">
      ...
      <button className="finish-btn" style={{width:"auto",margin:0,padding:"8px 18px",fontSize:15,letterSpacing:1}} onClick={handleFinish}>{allDone?"â DONE":"FINISH"}</button>
    </div>
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
      {exStates.map((exState,ei)=>{
        ...
        return(<div key={ex.id}>
          {pg&&!exState.skipped&&<div className={`ai-banner ${pg.type==="increase"?"good":""}`}>{pg.type==="increase"?`ð¥ Hit all reps 2 sessions in a row! Try ${(exState.weight+pg.amount).toFixed(2)}kg (+${pg.amount}kg)`:`ð¡ You struggled last session â stay at ${exState.weight}kg`}</div>}
          <div className={`ex-card ${exState.skipped?"ex-skip":allSD?"ex-done":""}`}>
            ...
            {!exState.skipped&&(
              <ExerciseSetTable ex={ex} sets={exState.sets} editable={true}
                ...
                onFieldBlur={(si)=>{
                  const r=parseInt(exState.sets[si].reps);
                  const isLastSet=si===exState.sets.length-1;
                  const isLastEx=ei===workout.exercises.length-1;
      ...
      <div className="note-wrap" style={{marginTop:8}}>
        ...
        <textarea className="note-ta" placeholder="Feeling a bit tired todayâ¦" value={note} onChange={e=>setNote(e.target.value)}/>
      </div>
      <button className="finish-btn" onClick={handleFinish}>{allDone?"â FINISH WORKOUT":"FINISH WORKOUT (EARLY)"}</button>
    </div>
  </div>);
}

// ââ HISTORY DETAIL âââââââââââââââââââââââââââââââââââââââââââ
  ...
  return(<div style={{display:"flex",flexDirection:"column",minHeight:"100svh"}}>
    <div className="w-header">
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:12,color:"var(--muted)"}}>{new Date(session.date).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})} Â· {session.duration}min</div>
      </div>
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
      ...
      {session.note&&<div style={{margin:"0 14px 14px",padding:"10px 13px",background:"var(--s2)",borderRadius:10,fontSize:13,color:"var(--muted2)",fontStyle:"italic"}}>ð {session.note}</div>}
    </div>
  </div>);
}

// ââ TRAINER CHAT âââââââââââââââââââââââââââââââââââââââââââââ
function TrainerChat({history,program,user}){
  const [msgs,setMsgs]=useState([{role:"ai",text:`Hey ${user?.name?.split(" ")[0]??"there"}! I'm your personal trainer. I know all your workout data. Ask me anything ðª`,time:nowT()}]);
  ...
  return(<div className="chat-wrap">
    ...
    <div className="chat-bar">
      <input className="chat-in" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Ask your trainerâ¦"/>
