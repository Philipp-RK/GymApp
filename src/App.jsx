...
.set-box{flex:1;border:1.5px solid var(--border);border-radius:12px;background:var(--s2);overflow:hidden;transition:border-color .15s,background .15s;}
  ...
  return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}>
    ...
    {day.exercises.map(ex=>(<div key={ex.id} style={{background:"var(--s2)",borderRadius:10,padding:12,marginBottom:8}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
        <div><div style={{fontSize:10,color:"var(--muted)",marginBottom:3}}>Sets</div><input type="number" className="mini-input" value={ex.sets} onChange={e=>{const val=Math.max(1,parseInt(e.target.value)||1);const resize=arr=>Array.from({length:val},(_,i)=>arr[i]??arr[0]??120);onUpdateEx(ex.id,{sets:val,weightPerSet:resize(ex.weightPerSet||Array(ex.sets).fill(ex.weight)),repsPerSet:resize(ex.repsPerSet||Array(ex.sets).fill(ex.reps)),restPerSet:resize(ex.restPerSet||Array(ex.sets).fill(ex.rest??120))});}}/></div>
        <div><div style={{fontSize:10,color:"var(--muted)",marginBottom:3}}>Default weight (kg)</div><input type="number" className="mini-input" value={ex.weight} onChange={e=>onUpdateEx(ex.id,{weight:parseFloat(e.target.value)||0})}/></div>
      </div>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr>{["Set","Reps","Weight (kg)","Rest (s)"].map(h=>(<th key={h} style={{fontSize:10,color:"var(--muted)",fontWeight:500,padding:"2px 4px",textAlign:"center"}}>{h}</th>))}</tr></thead>
          <tbody>{Array.from({length:ex.sets},(_,si)=>(
            <tr key={si}>
              <td style={{textAlign:"center",padding:"3px 2px",color:"var(--muted2)",fontSize:11}}>#{si+1}</td>
              <td style={{padding:"3px 3px"}}><input type="number" className="mini-input" value={(ex.repsPerSet||[])[si]??ex.reps} onChange={e=>{const a=[...(ex.repsPerSet||Array(ex.sets).fill(ex.reps))];a[si]=parseInt(e.target.value)||0;onUpdateEx(ex.id,{repsPerSet:a});}}/></td>
              <td style={{padding:"3px 3px"}}><input type="number" className="mini-input" value={(ex.weightPerSet||[])[si]??ex.weight} onChange={e=>{const a=[...(ex.weightPerSet||Array(ex.sets).fill(ex.weight))];a[si]=parseFloat(e.target.value)||0;onUpdateEx(ex.id,{weightPerSet:a});}}/></td>
              <td style={{padding:"3px 3px"}}><input type="number" className="mini-input" value={(ex.restPerSet||[])[si]??ex.rest??120} onChange={e=>{const a=[...(ex.restPerSet||Array(ex.sets).fill(ex.rest??120))];a[si]=parseInt(e.target.value)||0;onUpdateEx(ex.id,{restPerSet:a});}}/></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function WorkoutSession({workout,history,onFinish,onBack,startRest,restEnabled}){
  ...
  const [exStates,setExStates]=useState(()=>workout.exercises.map(ex=>({
    ...
    sets:Array.from({length:ex.sets},(_,si)=>({reps:"",targetReps:(ex.repsPerSet||[])[si]??ex.reps,weight:(ex.weightPerSet||[])[si]??ex.weight,skipped:false,done:false}))
  })));
  ...
  const onRepsBlur=(ei,si)=>{
    ...
    if(!isNaN(r)&&r>0&&restEnabled&&!(isLastSet&&isLastEx)){const restSecs=(ex.restPerSet&&ex.restPerSet[si]!=null)?ex.restPerSet[si]:(ex.rest??120);startRest(restSecs);}
  };
  ...
  return(<div style={{display:"flex",flexDirection:"column",minHeight:"100svh"}}>
    ...
    <div style={{flex:1,overflowY:"auto",paddingBottom:90}}>
        ...
        return(<div key={ex.id}>
          ...
          <div className={`ex-card ${exState.skipped?"ex-skip":allSD?"ex-done":""}`}>
            <div className="ex-chead">
              <div><div className="ex-cname">{ex.name}</div><div className="ex-cmeta">{ex.sets} sets{restEnabled&&ex.restPerSet?` · ${ex.restPerSet.join("/")}s`:restEnabled&&ex.rest?` · ${ex.rest}s`:""}</div></div>
            ...
            {!exState.skipped&&<>
              <div className="weight-row">
                ...
                <div><div className="w-num">{exState.weight}kg</div><div className="w-lbl">DEFAULT WEIGHT</div></div>
                <div className="w-btn" onClick={()=>changeWeight(ei,1)}>+</div>
              </div>
              <div className="sets-row">
                {exState.sets.map((s,si)=>(<div key={si} className={`set-box ${s.done?"set-done":""} ${s.skipped?"set-skip":""}`}>
                  <div className="set-box-top"><span className="set-num">Set {si+1}</span><div className="set-done-ring"><Ic.Check/></div><div className="set-x" onClick={()=>toggleSetSkip(ei,si)}>✕</div></div>
                  <div className="set-target">target: <span>{s.targetReps}</span> · <span>{(ex.weightPerSet||[])[si]??exState.weight}kg</span></div>
                  <input className="set-input" type="number" placeholder={String(s.targetReps)} value={s.reps} disabled={s.skipped} onChange={e=>onRepsChange(ei,si,e.target.value)} onBlur={()=>onRepsBlur(ei,si)}/>
                </div>))}
              </div>
            </>}
          </div>
      ...
      <div className="note-wrap"><div className="note-lbl">📝 Session note (optional)</div><textarea className="note-ta" placeholder="Feeling a bit tired today…" value={note} onChange={e=>setNote(e.target.value)}/></div>
