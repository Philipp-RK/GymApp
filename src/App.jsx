...
// Returns display label for a set: "W" for warmup, "D" for drop, sequential number for normal
function getSetLabel(types, idx) {
  const t = types[idx] ?? "normal";
  if (t === "warmup") return "W";
  if (t === "drop") return "D";
  // count only normal sets up to this index
  let n = 0;
  for (let i = 0; i <= idx; i++) {
    if ((types[i] ?? "normal") === "normal") n++;
  }
  return String(n);
}
...
function SetRow({ sets, si, editable=false, onTypeClick, onWeightChange, onRepsChange, onFieldBlur, onSkip, children }) {
  const s = sets[si];
  ...
  return (
    <tr className={`set-row ${s.done?"row-done":""} ${s.skipped?"row-skip":""} ${isWarmup&&!s.done?"row-warmup":""} ${isDrop&&!s.done?"row-drop":""}`}>
      ...
      <td style={{paddingLeft:14,paddingRight:6,width:40}}>
        <div onClick={editable&&onTypeClick?()=>onTypeClick(si):undefined}
          style={{width:32,height:32,borderRadius:8,background:typeBg,border:`1.5px solid ${typeColor}44`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontFamily:"var(--df)",fontSize:16,letterSpacing:.5,color:typeColor,
            cursor:editable?"pointer":"default",userSelect:"none",flexShrink:0,transition:"all .15s"}}>
          {label}
        </div>
      </td>
      {children}
...
export default function App() {
  ...
  if(viewingSession) return(<><style>{CSS}</style><div className="app">
    <HistoryDetail session={viewingSession} program={program} onBack={()=>setHistDetail(null)}/>
  </div></>);
...
function ExerciseSetTable({ ex, sets, editable=false, onTypeToggle, onWeightChange, onRepsChange, onFieldBlur, onSkip, onCheck, onAddSet }) {
  const thead = (
            ...
            <tr key={si} className={`set-row ${s.done?"row-done":""} ${s.skipped?"row-skip":""} ${isW&&!s.done?"row-warmup":""} ${isD&&!s.done?"row-drop":""}`}>
              ...
              <td style={{paddingLeft:14,paddingRight:4,width:44}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  <div onClick={editable&&onTypeToggle?()=>onTypeToggle(si):undefined}
                    style={{width:32,height:32,borderRadius:8,background:typeBg,
    ...
    {day.exercises.map(ex=>{
      ...
      return(
        <div key={ex.id} style={{background:"var(--s2)",borderRadius:12,marginBottom:10,overflow:"hidden"}}>
          ...
          />
          <button className="add-set-btn" onClick={()=>handleAddSet(ex.id)}><Ic.Plus/> Add Set</button>
        </div>
      );
    })}
          ...
          <div className={`ex-card ${exState.skipped?"ex-skip":allSD?"ex-done":""}`}>
            ...
            {!exState.skipped&&(
              <ExerciseSetTable ex={ex} sets={exState.sets} editable={true}
                onTypeToggle={si=>{
                  updSet(ei,si,{setType:cycleSetType(exState.sets[si].setType??"normal")});
                }}
