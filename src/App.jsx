...
function SetTypePicker({sets, si, onSelect}) {
  ...
  useEffect(() => {
    if (!pos) return;
    const fn = (e) => { if (btnRef.current && !btnRef.current.contains(e.target)) setPos(null); };
        ...
        <div className="type-picker-popup" style={{top:pos.top,left:pos.left}} onClick={e=>e.stopPropagation()}>
          {options.map(o => (
            <div key={o.type} className="type-picker-item"
              onClick={() => { onSelect(o.type); setPos(null); }}>
  ...
  const thead = (
    <thead className="set-table-head">
      <tr>
        ...
        <th>KG</th>
        <th>REPS</th>
        {editable && onRemoveSet && <th style={{textAlign:"right",paddingRight:10,fontSize:9}}>REST(s)</th>}
