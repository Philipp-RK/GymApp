...
function SetTypePicker({sets, si, onSelect}) {
  ...
  return (
    <div className="type-picker-wrap" ref={ref}>
      ...
      {open && (
        <div className="type-picker-popup" style={{bottom:38}} onClick={e=>e.stopPropagation()}>
          {options.map(o => (
            <div key={o.type} className="type-picker-item"
              onClick={() => { onSelect(o.type); setOpen(false); }}>
...
export default function App() {
  ...
  if(histDetail) return(<><style>{CSS}</style><div className="app">
    <HistoryDetail session={histDetail} program={program} onBack={()=>setHistDetail(null)}/>
  </div></>);
...
function ExerciseSetTable({ ex, sets, editable=false, onTypeSelect, onWeightChange, onRepsChange, onFieldBlur, onSkip, onRemoveSet, onCheck, onAddSet }) {
  const thead = (
    <thead className="set-table-head">
      <tr>
        ...
        <th>KG</th>
        <th>REPS</th>
        {editable && <th style={{textAlign:"right",paddingRight:10}}></th>}
      </tr>
    </thead>
  );
              ...
              {editable && (
                <td style={{paddingRight:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end"}}>
                    ...
                    <div className="set-skip-x" onClick={()=>onRemoveSet?onRemoveSet(si):onSkip&&onSkip(si)} title={onRemoveSet?"Remove set":s.skipped?"Restore":"Skip"}>
                      {onRemoveSet?"✕":s.skipped?"↩":"✕"}
                    </div>
                  </div>
                </td>
              )}
        ...
        return(<div key={ex.id}>
          ...
          <div className={`ex-card ${exState.skipped?"ex-skip":allSD?"ex-done":""}`}>
            <div className="ex-chead">
              <div><div className="ex-cname">{ex.name}</div><div className="ex-cmeta">{exState.sets.filter(s=>!s.skipped&&s.setType==="normal").length} working sets</div></div>
...
// ── HISTORY DETAIL ───────────────────────────────────────────
function HistoryDetail({session,program,onBack}){
  const day=program.find(d=>d.id===session.dayKey);
  return(<div style={{display:"flex",flexDirection:"column",minHeight:"100svh"}}>
    <div className="w-header">
      <button className="back-btn" onClick={onBack}><Ic.Back/></button>
