    ...
    <div className="card" style={{userSelect:"none",touchAction:"pan-y"}}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>
        {days.map(({day, date, status, isToday}, i) => {
          ...
          const letterColor = isRest ? "var(--border2)"
            : status === "done" ? "#4CAF50"
          ...
          return (
            <div key={i} style={{
              ...
              background: bg,
              border: isToday ? "1.5px solid var(--accent)" : "1px solid " + (isRest ? "var(--border)" : "var(--border)"),
            }}>
              <div style={{fontSize:9,color:"var(--muted)",letterSpacing:.3}}>{DAY_NAMES[i]}</div>
              <div style={{fontFamily:"var(--df)",fontSize:isRest?18:14,color:letterColor,lineHeight:1}}>{letter}</div>
              <div style={{fontSize:8,color:"var(--muted)",lineHeight:1}}>{date.getDate()}/{date.getMonth()+1}</div>
              {statusDot[status] && !isRest && (
                <div style={{width:4,height:4,borderRadius:"50%",background:statusDot[status]}}/>
              )}
            </div>
          );
        })}
  ...
  return(
    <div key={ex.id} className="ex-row" style={{alignItems:"center"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
        ...
        <span style={{fontSize:12,color:"var(--muted2)",whiteSpace:"nowrap"}}>{displayWeight}kg · {displayReps} reps</span>
      </div>
    </div>
  );
})}</div>
